...
```
git clone https://github.com/tsubame-rustica/tbr-summer-fes.git
```
を実行する
...
#### ブランチを作成しpushする
1.  作業前に`git pull origin master`でローカルのmainブランチを最新化する
ある程度の進捗を生み出したらリポジトリにpushします  
2.  `git checkout -b 作成したいブランチ名`を実行しブランチを作成する
ブランチ名はバージョンや日付など誰もが分かりやすい名前を付ける  
例）`git checkout -b 01-01-00-00`（1月1日0時0分に作成した場合）
3.  `git add .`を実行
4.  `git commit -m "コミットメッセージ（任意の文字）"`を実行しコミットする
5.  `git push origin ブランチ名`を実行しpushする
6.  pushが完了したらpull requestが送られるのでその旨を権限（リーダー等）のある人に伝える
